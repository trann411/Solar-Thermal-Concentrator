...
void ATControl_PinConfigure(char targetMotor, char direction, double dutyCycle) {  // Duty Cycle (25% to 100%)
  ...
  if (targetMotor == 'A') {
    ...
    if (direction == 'F') {
      // SET IN1/IN3 high, IN2/IN4 blow; break before make to prevent magic smoke
      digitalWrite(A4, LOW);
...
double ATControl_PyroSweep(int _pyroPin, double presentAngle) {
  ...
  for (int i = 0; i < numberOfLoopIterations; ++i) {
    ...
    pyroValue = pyroValue / numberOfPyroSamples;
    ...
    pyroValue = 0; // Reset average for next loop iteration
