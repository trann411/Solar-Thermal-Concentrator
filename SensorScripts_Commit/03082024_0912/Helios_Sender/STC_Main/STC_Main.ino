...
class MyCallbacks : public BLECharacteristicCallbacks {  // Interface Read/Write Function to Return Data
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (pch1 == "na") {
      ...
      if (pch0 == "magno") {  // Query Tilt Angle
      ...
      if (pch0 == "azimuthstage") {  // Take in Azimuth Angle
        ...
        ATControl_MagnoRotate(targetAngle, mag);
        //static char azimuthHeading[6];
        //dtostrf(azimuthAngle, 6, 2, azimuthHeading);
        //pCharacteristic->setValue(azimuthHeading);
      }

      if (pch0 == "tiltstage1") {  // Take in Azimuth Angle
        double targetAngle = pch1.toDouble();
        pCharacteristic->setValue("Rotating Stage 1 Tilt");
        ATControl_AccelRotate(targetAngle, mma2, mma3, mma4, 1);
        pCharacteristic->setValue("Tilt reached");
      }

      if (pch0 == "tiltstage2") {  // Take in Azimuth Angle
        double targetAngle = pch1.toDouble();
        pCharacteristic->setValue("Rotating Stage 2 Tilt");
      ...
      if (pch0 == "tiltstage3") {  // Take in Azimuth Angle
        double targetAngle = pch1.toDouble();
        pCharacteristic->setValue("Rotating Stage 3 Tilt");
        ATControl_AccelRotate(targetAngle, mma2, mma3, mma4, 3);
        pCharacteristic->setValue("Tilt reached");
      }

      if (pch0 == "kalmanangle") {  // UPDATE WITH MORE RECENT ALGORITHM
        float angleToInput;
        float currentTime;
        float timeElapsed;
        ...
        static char accelAngle[6];
        dtostrf(angleToReturn, 6, 2, accelAngle);
        pCharacteristic->setValue(accelAngle);
      }
    }
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (command.equals("halt")) {
      ...
      while (!command.equals("halt")) {
        Serial.print("Magno: ");
        double reading = Magno_CompassSample(mag);
