...
float Accel_KalmanFilter(float tiltArrayAngle) {
  testRate = 0; // Gyroscopic Rate, not utilized
  testDt = 1; // Time Between Samples
  testOutAngle = filter.getAngle(tiltArrayAngle, testRate, testDt); // Angle from Accel Array
