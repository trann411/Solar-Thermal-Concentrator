...
#include "RotaryEncode.h"           // Rotary Encoder Functionsa
...
/* Magnetometer Variables */  // Magnotometer no longer utilized, but keep declaration in for legacy (can remove, if you take the time)
...
String pch0temp;
String pch1temp;
char *bluetoothReturnData = "NULL";
char bluetoothRawType[6] = "NULL";
char bluetoothRawData[6] = "NULL";
char bluetoothConcatData[12] = "NULL";
char *clearBluetoothReturnData = "";
char clearBluetoothRawData[6] = "";
...
float currentTime = 0;
float timeElapsed = 0;
double shutterStartTime = 0;
double shutterElapsedTime = 0;
/* Miscellaneous Global Variables */
...
class MyCallbacks : public BLECharacteristicCallbacks {  // Interface Read/Write Function to Return Data
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (delimiterIndex != -1) {
      pch0temp = input.substring(0, delimiterIndex);
      pch1temp = input.substring(delimiterIndex + 1, input.length());
    } else {
      pch0temp = input;
      pch1temp = "na";
    }

    if (bluetoothReturnData != "Busy") {  //Reject all incoming input while busy to prevent command bleed-over
      pch0 = pch0temp;
      pch1 = pch1temp;
...
void setup() {
  ...
  /* Initialize Magnetometer * // Magnotometer no longer utilized
  ...
  BLEDevice::init("Helios_ESP32");  // Create Server & Server Name
...
void loop() {
  ...
  if (doThermo) {
    ...
    memset(bluetoothReturnData, 0, sizeof(bluetoothRawData));  // Clear return data to channel to prevent overlapping values
    memset(bluetoothRawData, 0, sizeof(bluetoothRawData));

    dtostrf(temp, 6, 2, bluetoothRawData);  // Convert value to char
    strcpy(bluetoothReturnData, "temp,");   // Concaternate datatype flag with data and push to return channel
    strcat(bluetoothReturnData, bluetoothRawData);
  ...
  if (doPyro) {
    ...
    double pyroReadback = Pyro_ReadTemp(A1);
    //dtostrf(temp, 6, 2, bluetoothReturnData);
    memset(bluetoothReturnData, 0, sizeof(bluetoothRawData));  // Clear return data to channel to prevent overlapping values
    memset(bluetoothRawData, 0, sizeof(bluetoothRawData));

    dtostrf(pyroReadback, 6, 2, bluetoothRawData);  // Convert value to char
    strcpy(bluetoothReturnData, "pyro,");           // Concaternate datatype flag with data and push to return channel
    strcat(bluetoothReturnData, bluetoothRawData);
    doPyro = false;
  }
  ...
  if (doMagno) {  // Query Azimuth Angle
    ...
    //dtostrf(presentAngle, 6, 2, bluetoothRawData);
    //bluetoothReturnData = strcat("magn,", bluetoothRawData);

    memset(bluetoothReturnData, 0, sizeof(bluetoothRawData));  // Clear return data to channel to prevent overlapping values
    memset(bluetoothRawData, 0, sizeof(bluetoothRawData));

    dtostrf(presentAngle, 6, 2, bluetoothRawData);
    strcpy(bluetoothReturnData, "magn,");  // Concaternate datatype flag with data and push to return channel
    strcat(bluetoothReturnData, bluetoothRawData);
  ...
  if (doAccelArray) {  // Query Tilt Angle
    ...
    //dtostrf(angle, 6, 2, bluetoothRawData);
    //bluetoothReturnData = strcat("accl,", bluetoothRawData);
    ...
    dtostrf(angle, 6, 2, bluetoothRawData);
    strcpy(bluetoothReturnData, "accl,");  // Concaternate datatype flag with data and push to return channel
    strcat(bluetoothReturnData, bluetoothRawData);
  ...
  if (doLevel) {
    strcpy(bluetoothReturnData, "Busy");
    ...
    doLevel = false;
    strcpy(bluetoothReturnData, "Done");
  }
  ...
  if (doKalmanAngle) {  // UPDATE WITH MORE RECENT ALGORITHM
    ...
    //dtostrf(angleToReturn, 6, 2, bluetoothRawData);
    //bluetoothReturnData = strcat("kalf,", bluetoothRawData);

    memset(bluetoothReturnData, 0, sizeof(bluetoothRawData));  // Clear return data to channel to prevent overlapping values
    memset(bluetoothRawData, 0, sizeof(bluetoothRawData));

    dtostrf(angleToReturn, 6, 2, bluetoothRawData);
    strcpy(bluetoothReturnData, "kalm,");  // Concaternate datatype flag with data and push to return channel
    strcat(bluetoothReturnData, bluetoothRawData);
  ...
  if (doPyroSweep) {
    strcpy(bluetoothReturnData, "Busy");
    ATControl_PyroSweep(A1, presentAngle);
    presentAngle = pch1.toDouble();  // Set present angle after sweep finishes with value from BLE
    doPyroSweep = false;
    strcpy(bluetoothReturnData, "Done");
  }
  ...
  if (Serial.available()) {
    ...
    if (command.equals("halt")) {
      ...
      presentAngle = ATControl_PyroSweep(A1, presentAngle);
    }
    ...
    else if (command.equals("shutteropen")) {
      Serial.println("Opening Shutters.");
      shutterStartTime = millis();
      shutterElapsedTime = millis() - shutterStartTime;
      while (shutterElapsedTime < 6000) {
        SHControl_PinConfigure('R', 'F', 80);
        SHControl_PinConfigure('L', 'F', 80);
        shutterElapsedTime = millis() - shutterStartTime;
        Serial.println(shutterElapsedTime);
      }
      SHControl_PinConfigure('R', 'S', 0);
      SHControl_PinConfigure('L', 'S', 0);
      Serial.println("Finished opening shutters.");
    }

    else if (command.equals("shutterclose")) {
      Serial.println("Closing Shutters.");
      shutterStartTime = millis();
      shutterElapsedTime = millis() - shutterStartTime;
      while (shutterElapsedTime < 6000) {
        SHControl_PinConfigure('R', 'B', 80);
        SHControl_PinConfigure('L', 'B', 80);
        shutterElapsedTime = millis() - shutterStartTime;
        Serial.println(shutterElapsedTime);
      }
      SHControl_PinConfigure('R', 'S', 0);
      SHControl_PinConfigure('L', 'S', 0);
      Serial.println("Finished closing shutters.");
    }
