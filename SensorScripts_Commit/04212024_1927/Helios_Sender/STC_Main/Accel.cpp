...
#define TCAADDR 0x70 // I2C Address for MUX
...
float Accel_ArrayTiltAngle(Adafruit_MMA8451 mma2, Adafruit_MMA8451 mma3, Adafruit_MMA8451 mma4) {
  ...
  double Cout = Accel_TiltAngle(mma3, 3);
  double Bout = Accel_TiltAngle(mma4, 4);
  ...
  double mechanicalMountComp = 0;  // Calibration factor (degrees) for angle at which array is mounted on concentrator
  ...
  /* For use on concentrator, only travels through Cases I - III*/
  ...
  else if ((0 < Aout) && (absBout <= halfg) && (Cout < 0)) {  // Case II: 0<Aout, |Bout|<=0.5g, Cout<0
    //Serial.println("Tilt Angle Case: II");
    angle = (-1 * asin(Bout / g)) + PI - thetaAB - ((3*PI)/180);  // θ = -arcsin(Bout/g) + π – θAB - angle offset due to mounting
    //angle = angle + (6 * (PI / 180));              //Offset
  }

  else if ((0 < Aout) && (Bout < 0) && (absCout <= halfg)) {  // Case III: 0<Aout, Bout<0, |Cout|<=0.5g
    //Serial.println("Tilt Angle Case: III");
    angle = asin(Cout / g) + (2 * PI) - thetaAB - thetaBC;  // θ = arcsin(Cout/g) + 2π – θAB - θBC
    //angle = PI - (angle - (4 * PI / 3));                    //Offset
  }

  else if ((absAout <= halfg) && (Bout < 0) && (0 < Cout)) {  // Case IV: |Aout|<=0.5g, Bout<0, 0<Cout
    //Serial.println("Tilt Angle Case: IV");
    angle = (-1 * asin(Aout / g)) + PI;  // θ= -arcsin(Aout/g)+ π
    //angle = -1 * (angle - PI);           //Offset
  }

  else if ((Aout < 0) && (absBout <= halfg) && (0 < Cout)) {  // Case V: Aout<0, |Bout|<=0.5g, 0<Cout
    Serial.println("Tilt Angle Case: V");
    angle = asin(Bout / g) + (2 * PI) - thetaAB;  // θ= arcsin(Bout/g)+2π-θAB
  }

  else if ((Aout < 0) && (0 < Bout) && (absCout <= halfg)) {  // Case VI: Aout<0, 0<Bout, |Cout|<=0.5g
    Serial.println("Tilt Angle Case: VI");
    angle = (-1 * asin(Cout / g)) + (3 * PI) - thetaAB - thetaBC;  // θ= -arcsin(Cout/g)+3π-θAB-θBC
  }

  else if ((neghalfg <= Aout) && (Aout < 0) && (0 < Bout) && (Cout < 0)) {  // Case VII: -0.5g<=Aout<0, 0<Bout, Cout<0
  ...
  //angle = 180 - angle;
  angle *= (180 / PI); // Convert radians to degrees
  angle = (1.0126 * angle) + 0.996; // Linearly interpolated calibration formula from comparing with Klein 934DAG Angle Meter
  //Serial.print("Angle: ");
