...
void loop() {
  ...
  if (Serial.available()) {
    ...
    else if (command.equals("magnofeedbacktilt")) {
      Serial.println("Starting Magnotometer Feedback Loop");
      while (!command.equals("halt")) {
        ATControl_MagnoRotate(55, mag, 20);
        if (Serial.available()) {
          command = Serial.readStringUntil('\n');
        }
      }
    ...
    Serial.println("Starting Accelerometer Feedback Loop");
    while (!command.equals("halt")) {
      ATControl_AccelRotate(25, mma2, mma3, mma4, 20);
      if (Serial.available()) {
        command = Serial.readStringUntil('\n');
      }
    }
