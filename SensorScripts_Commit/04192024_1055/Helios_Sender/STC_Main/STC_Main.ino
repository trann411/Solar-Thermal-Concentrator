...
void loop() {
  ...
  if (doThermo) {
    ...
    dtostrf(temp, 6, 2, bluetoothRawData);
    strcpy(bluetoothReturnData, ""); // Clear before concaternating to avoid appending onto old data.
    bluetoothReturnData = strcat("", "temp,");
    bluetoothReturnData = strcat("", bluetoothRawData);
  ...
  if (doMagno) {  // Query Azimuth Angle
    ...
    strcpy(bluetoothReturnData, ""); // Clear before concaternating to avoid appending onto old data.
    bluetoothReturnData = strcat("", "magn,");
    bluetoothReturnData = strcat("", bluetoothRawData);
  ...
  if (doAccelArray) {  // Query Tilt Angle
    ...
    strcpy(bluetoothReturnData, ""); // Clear before concaternating to avoid appending onto old data.
    bluetoothReturnData = strcat("accl,", "");
    bluetoothReturnData = strcat("", bluetoothRawData);
  ...
  if (doAzimuthStage) {  // Take in Azimuth Angle
    ...
    dtostrf(presentAngle, 6, 2, bluetoothRawData);
    bluetoothReturnData = strcat("azif,", bluetoothRawData);
