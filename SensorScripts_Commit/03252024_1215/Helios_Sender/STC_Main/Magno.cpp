...
Kalman magnofilter;
float magnotestRate = 0;
float magnotestOutAngle = 0;
/* /Kalman Filter Object */
...
void Magno_StartKalmanFilter(Adafruit_MMC5603 mag) {
  float startAngle = Magno_CompassSample(mag);
  magnofilter.setAngle(startAngle);
}

float Magno_KalmanFilter(float magnoCompassAngle, float timeBetweenSamples) {
  magnotestRate = 0;                                                                  // Gyroscopic Rate, not utilized
  magnotestOutAngle = magnofilter.getAngle(magnoCompassAngle, 0, timeBetweenSamples);  // Angle from Accel Array
  return magnotestOutAngle;
}
/* /Post-Processing to Derive More Accurate Reading from Noisy Angle Data */
