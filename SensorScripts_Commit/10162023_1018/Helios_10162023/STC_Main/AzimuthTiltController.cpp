...
void ATControl_PinConfigure(char targetMotor, char direction) {
  ...
  if (targetMotor == 'A') {
    ...
    ATControl_PWMPulse(0, PWMDutyCycle(100));  // PWM Pulse Channel & Percentage Duration
  }
  ...
  if (targetMotor == 'T') {
    ...
    ATControl_PWMPulse(2, PWMDutyCycle(100));
  }
}
...
void ATControl_Rotate(double targetAngle, Adafruit_MMC5603 mag) {
  ...
  double angleBounds[] = { targetAngle * 1.05, targetAngle * 0.95 };
  ...
  while (currentAngle != targetAngle) {
    ...
    else if (currentAngle < angleBounds[1]) {
