...
double RotaryEncode_Tracking(int numberOfSamples) {
  int pastA = -1; // Initialize to non-realizable state.
  int pastB = -1;
  int presA = -1;
  int presB = -1;
  ...
  analogpresA = analogRead(RX);  // digitalRead() yields wrong values; do analogRead() and manually set threshold for 0 and 1.
  analogpresB = analogRead(TX);

  if (analogpresA >= 2) { // 2 volts is threshold for determining 0 or 1.
    presA = 1;
  }
  if (analogpresA < 2) {
    presA = 0;
  ...
  while (isRotaryTargetReached == false) {
    ...
    // Brute-force behavioral description based on recorded states; for determing direction of encoder rotation.
    ...
    Serial.print(presB);
    Serial.print(", RX (A) V: ");
    Serial.print(analogpresA);
    Serial.print(", TX (B) V: ");
    Serial.print(analogpresB);
    Serial.print(", DoR = ");
