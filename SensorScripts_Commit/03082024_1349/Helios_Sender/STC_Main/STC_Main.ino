...
char bluetoothReturnData[6] = "Test";
...
void loop() {
  ...
  if (doAzimuthStage) {  // Take in Azimuth Angle
    ...
    double azimuthAngle = ATControl_MagnoRotate(targetAngle, mag);
    dtostrf(azimuthAngle, 6, 2, bluetoothReturnData);
    ...
    if (command.equals("halt")) {
      ...
      while (!finished) {
        ...
        if (Serial.available()) {
          ...
          if (!command.equals("")) {
            ATControl_MagnoRotate(desiredAngle, mag);
            finished = true;
          }
        }
      }
