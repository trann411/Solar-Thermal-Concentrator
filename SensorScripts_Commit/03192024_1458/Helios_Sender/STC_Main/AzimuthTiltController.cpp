...
double ATControl_PyroSweep(int _pyroPin, double presentAngle) {
  ...
  double angleToIncrementBy = 0.1;
  int numberOfLoopIterations = round(180 / angleToIncrementBy);
  ...
  for (int i = 0; i < numberOfLoopIterations; ++i) {
    ...
    presentAngle = ATControl_MagnoRotate(angleToIncrementBy, presentAngle);
