...
void ATControl_MagnoRotate(double targetAngle, Adafruit_MMC5603 mag) {  // NEED TO ADDRESS EDGE CASES AROUND 0 DEG TARGET ANGLE
  double dutyCycle = 20;
  int numberOfSamples = 30; // Numbers of samples for rotary encoder to take; ensure that (90 / PPR) * numberSamples <= 0.1 to keep within accuracy targets.

  double presentAngle = 90;
  double angleDelta = 0;
...
void ATControl_AccelRotate(double targetAngle, Adafruit_MMA8451 mma2, Adafruit_MMA8451 mma3, Adafruit_MMA8451 mma4, int desiredStage) {  // NEED TO ADDRESS EDGE CASES AROUND 0 DEG TARGET ANGLE
  ...
  if (desiredStage == 3) {
    ...
    while (isStageThreeTargetReached == false) {
      ...
      for (int i = 0; i < 30; ++i) {  //Take multiple samples to avoid skew by bad value
        currentTime = millis();
        timeElapsed = (currentTime - startTime) / 1000.0000;
      ...
      if (currentAngle <= stageThreeAngleBounds[0]) {
        if (currentAngle >= stageTwoAngleBounds[1]) {
          ...
          stageThreeCounter += 1;
          if (stageThreeCounter >= 10) {  // If 100 good values read in from kalmanfilter, allow to pass on to next stage
      ...
      if (currentAngle < stageThreeAngleBounds[1]) {
        ...
        delay(750);  // 100 is another sweet spot
        ATControl_PinConfigure('t', 's', stageThreeDutyCycle);
        stageThreeCounter = 0;  // Reset good samples counter
        // Delay & wait to stabilize
        delay(500);
